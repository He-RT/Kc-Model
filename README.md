...
## 空间尺度与数据一致性 (2026-05-13)

### 站点训练中的尺度错配

站点 Kcact = ETa / ET0，但分子分母不在同一空间尺度上：

| 变量 | 来源 | 原生分辨率 | 代表面积 | vs 通量足迹 |
|------|------|-----------|---------|------------|
| ETa（分子） | 涡度相关/蒸渗仪 | ~100-500m 足迹 | ~0.1 km² | **基准** |
| ET0（分母） | ERA5-Land 气象 → FAO-56 PM | 0.1° (~11km) | ~100 km² | **100-3000x** |
| NDVI | MOD13Q1 | 250m | 0.06 km² | 1-25x |
...
同坐标、同时段、同 ET0 分母，对比 MOD16A2GF 的 Kc 和通量塔 Kc：
...
| r(ETc 原始值) | **0.47** |
| Kc_tower 均值 | 0.55 |
| Kc_mod16 均值 | 0.40 (**低 29%**) |
| 夏季 r(ETc) | 0.24（最差） |
| 馆陶站 | r=0.63（最好），偏差仅 +0.02 |
| 栾城站 | MOD16 无数据覆盖 |

**结论：MOD16 500m ET 和通量塔实测在点位尺度上近乎不相关。** 大样本模型 R²=0.77 学到的是 MOD16 PM 算法的内部规律，不是真正的田间作物蒸散。站点模型 R²=0.47 低的原因之一是目标（通量塔 Kc）更准确、更难预测。

详见 `outputs/tables/kc_mod16_vs_tower.csv` 和 `scripts/python/compare_kc_mod16_vs_tower.py`。

### 提高 ET0 分辨率的路径

| 方案 | 分辨率提升 | 难度 | 备注 |
|------|----------|------|------|
| 站级气象观测 → FAO-56 PM | 11km → 点 | 低（如果有数据） | 最优，需确认 CERN 气象数据可得性 |
| CMFD (中国区域强迫数据) | 无提升（仍 0.1°） | 中 | 比 ERA5 精度高，GEE 无，需 TPDC 下载 |
| DEM 降尺度（气温递减率） | 11km → ~100m | 中 | 物理合理，可批量。华北平原平坦，辐射校正需求小 |
...
| 来源 | 站点 | 方法 | 可靠性 |
|------|------|------|--------|
| Dai et al. 2013 (TPDC) | 全国 30" (~1km) | 多 PTF 集成 | 准确度未知（作者自述），栾城系统性低 50% |
| Saxton-Rawls 2006 PTF | 馆陶（图推算） | 砂+黏+OM → FC/WP | 修正版（原论文系数 0.0452→0.452） |
| 实测 | 栾城、泰安 | 压力板/vG 方程 | 最可靠 |
| 文献 | 封丘、禹城 | 实测/vG | 可靠 |

### 馆陶 FC/WP（Saxton-Rawls 2006 修正版）
...
| 26-45cm | 心土 | 56.6% | — | — | — |
| 45-100cm | 底土 | 30.1% | — | — | — |
| **0-100cm 加权** | | | **38.0%** (380mm) | **23.1%** (231mm) | **14.9%** (149mm) |

详见 `outputs/tables/meta_fc_wp.csv` （17 条记录，含站点和参考文献）。

## 土壤水分数据清单

| 数据集 | 时空分辨率 | 层次 | 站点状态 |
|--------|----------|------|---------|
| FLDAS (Noah01) | 月, 0.1° | 0-10, 10-40, 40-100, 100-200cm | 已提取 `data/processed/fldas_station_sm.csv` |
| ERA5-Land SM | 月/日, 0.1° | 0-7, 7-28, 28-100, 100-289cm | 已提取 `data/processed/era5_station_sm.csv`，SM_root 已写入 Excel |
| GLDAS SM | 月, 0.25° | 4 层 | 13 个 GEE 导出任务已提交，待下载 |
| ESA CCI SM | 日, 0.25° | 表层 | 未获取（需独立认证） |

## 夏玉米物候与品种

### 物候日期范围

| 站点 | 播种 | 抽雄（水分最敏感） | 收获 | 全生育期 |
|------|------|------------------|------|---------|
| 栾城 (最北) | 6/15-6/20 | 7/28-8/05 | 9/25-10/05 | ~105 天 |
| 禹城 | 6/12-6/18 | 7/25-8/02 | 9/22-10/01 | ~103 天 |
| 位山 | 6/12-6/18 | 7/25-8/02 | 9/22-10/01 | ~103 天 |
| 馆陶 (最南) | 6/10-6/15 | 7/22-7/30 | 9/20-9/28 | ~100 天 |

南北梯度约 5-7 天。南北纬度差 1.5°，积温差约 100°C·d。

### 品种

四个通量站的年际品种记录未在公开数据库中发现 (CERN/ScienceDB 不含田间管理数据)。文献推测主导品种为**郑单 958** (2004+)，此前为农大 108。站间品种差异对物候节点的附加影响约 2-5 天，小于纬度梯度的 5-7 天。
